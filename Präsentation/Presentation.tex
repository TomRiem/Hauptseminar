...
\begin{frame}{Symmetric Rank One Update}
    We set
    \begin{align*}
        & s_k = \eta_k \in \tangent{x_k} \; (\widetilde{x}_{k+1} = R_{x_k}(\eta_k))\\
    ...
    where $\nu \in (0,1)$, then
    \begin{equation*}
        \widetilde{\mathcal{H}}_{k+1} [\cdot] = \mathcal{H}_k [\cdot] + \frac{(y_k - \mathcal{H}_k [s_k]) (y_k - \mathcal{H}_k [s_k])^{\flat} [\cdot] }{(y_k - \mathcal{H}_k [s_k])^{\flat} [s_k]},
    \end{equation*}
