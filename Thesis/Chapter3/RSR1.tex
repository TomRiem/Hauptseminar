...
We note that it is possible to define the update \cref{RiemannianDirectSR1formula} in the tangent space at $\tangent{\widetilde{x}_{k+1}}$, in this case the operator $\mathcal{H}^\mathrm{SR1}_k$ and $s_k$ would have to be transported to $\tangent{\widetilde{x}_{k+1}}$ and we would have to define $y_k$ in $\tangent{\widetilde{x}_{k+1}}$. The resulting algorithm would remain equivalent since the vector transport is isometric \cite[p.~5]{HuangAbsilGallivan:2014}. \\
...
\cref{RiemannianLocalConvergence} shows, that the $n + 1$-step q-superlinear convergence, \cref{n+1superlinear}, of the Euclidean SR1 method, \cref{TR-SR1Method}, is preserved when 

transferred to the Riemannian setup. we do not have strict assumptions regarding the choice of retraction


% Irgendwas noch mit "ach cool, die gleiche Konvergenzrate. Vorausetzung für retraction halbwegs allgemein. Trust-Region Approach mit SR1 anscheinend echt cool."

% Darauf verweisen, dass wir es RTR-SR1 nennen